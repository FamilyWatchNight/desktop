...
will copy the active test hooks from `./src/main/testing-active/`. For all other builds (especially production builds), the `use-testing-noop.js` prebuild script will copy the empty test hooks from `/src/main/testing-noop/`.