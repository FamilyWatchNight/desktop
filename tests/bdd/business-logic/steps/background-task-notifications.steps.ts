...
function getTaskRefName(world: CustomWorld, taskId: string): string | undefined {
  const objectStore = world.getStateObjectStore('tasks') as
    | { latest: unknown; all: Map<string, unknown> }
    | undefined;
...
When(
  'I attempt to enqueue a background task {string}',
  async function (this: CustomWorld, refName: string) {
...
When(
  'I set the task progress to current={int}, max={int}, description={string}',
  async function (this: CustomWorld, current: number, max: number, description: string) {
    const system = getSystemPersona(this);
...
When(
  'I set the task description to {string}',
...
Then(
  '{string} should be the active task',
  async function (this: CustomWorld, expectedRefName: string) {
    const system = getSystemPersona(this);
    const state = (await system.getTaskState()) as {
...
Then(
  'the most recent {string} event should have no active task',
...
Then(
  'the most recent {string} event should have active task with status={string}, current={int}, max={int}',
  async function (
    this: CustomWorld,
    eventType: string,
...
Then(
  'exactly {int} {string} events should be recorded',
...
Then(
  'the most recent {string} event should have {int} queued tasks',
  async function (this: CustomWorld, eventType: string, expectedQueueCount: number) {
    const system = getSystemPersona(this);
    const events = await system.filterEventsByType(eventType);
    expect(events.length).toBeGreaterThan(0);
...
Then(
  'the background task state should contain the active task',
...
Then('the active background task should be cancelled', async function (this: CustomWorld) {
  const result = this.getStateReturn('cancelActiveTask') as { success?: boolean } | undefined;
  expect(result).toBeDefined();
  expect(result?.success).toBe(true);
});

Then(
  'the queued background task should be removed successfully',
  async function (this: CustomWorld) {
    const result = this.getStateReturn('removeQueuedTask') as { success?: boolean } | undefined;
    expect(result).toBeDefined();
    expect(result?.success).toBe(true);
  },
);
