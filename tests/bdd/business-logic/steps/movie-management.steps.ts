...
Given(
  'stub Watchmode data is loaded from {string}',
  async function (this: CustomWorld, dataSource: string) {
...
Given(
  'stub TMDB data is loaded from {string}',
  async function (this: CustomWorld, dataSource: string) {
    const system = getSystemPersona(this);
...
When(
  'I look up the movie with TMDB ID {string}',
  async function (this: CustomWorld, tmdbId: string) {
    await getMovieByTmdbId(this, tmdbId);
  },
);

When(
  'I attempt to look up the movie with TMDB ID {string}',
  async function (this: CustomWorld, tmdbId: string) {
...
When(
  'I look up the movie with Watchmode ID {string}',
  async function (this: CustomWorld, watchmodeId: string) {
    await getMovieByWatchmodeId(this, watchmodeId);
  },
);

When(
  'I attempt to look up the movie with Watchmode ID {string}',
  async function (this: CustomWorld, watchmodeId: string) {
    await attemptAsync(this, async () => {
      await getMovieByWatchmodeId(this, watchmodeId);
    });
...
Then(
  'the original movie title should be {string}',
...
When(
  'I attempt to search movies by title {string}',
