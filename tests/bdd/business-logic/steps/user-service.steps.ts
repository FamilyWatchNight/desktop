...
import {
  generatePngBuffer,
  generateJpegBuffer,
} from '../../business-flow/utilities/image-generator';
...
When(
  'I create a user with username {string} and no password',
...
When(
  'I attempt to create a user with username {string} and no password',
  async function (this: CustomWorld, username: string) {
...
When(
  'I create a user with username {string} and password {string}',
...
When(
  'I attempt to create a user with username {string} and password {string}',
  async function (this: CustomWorld, username: string, password: string) {
    await attemptAsync(this, async () => {
      await createUserWithPassword(this, username, password);
    });
...
Given(
  'a user exists with username {string} and no password',
  async function (this: CustomWorld, username: string) {
...
Given(
  'a user exists with username {string} and password {string}',
  async function (this: CustomWorld, username: string, password: string) {
    const system = getSystemPersona(this);
    const user = await system.createUser({ username, password });
    setStoreUser(this, user, username);
...
When(
  'I authenticate with username {string} and no password',
  async function (this: CustomWorld, username: string) {
...
When(
  'I attempt to authenticate with username {string} and no password',
  async function (this: CustomWorld, username: string) {
    await attemptAsync(this, async () => {
      await authenticateNoPassword(this, username);
    });
...
When(
  'I authenticate with username {string} and password {string}',
  async function (this: CustomWorld, username: string, password: string) {
...
When(
  'I attempt to authenticate with username {string} and password {string}',
  async function (this: CustomWorld, username: string, password: string) {
    await attemptAsync(this, async () => {
      await authenticateWithPassword(this, username, password);
    });
...
Given(
  "the user's display name is {string}",
...
When(
  "I update the user's display name to {string}",
  async function (this: CustomWorld, displayName: string) {
    await updateUserProfile(this, undefined, displayName);
  },
);
...
When(
  'I attempt to update the display name of user {string} to {string}',
  async function (this: CustomWorld, username: string, displayName: string) {
    await attemptAsync(this, async () => {
      await updateUserProfile(this, username, displayName);
    });
  },
);

Then(
  "the user's display name should be {string}",
...
When(
  'I save a {int}KB PNG profile image for the user',
...
When(
  'I attempt to save a {int}KB PNG profile image for the user',
  async function (this: CustomWorld, size: number) {
    await attemptAsync(this, async () => {
      await saveProfileImage(this, undefined, size * 1024, 'image/png');
    });
...
When(
  'I attempt to save a {int}KB PNG profile image for the user {string}',
  async function (this: CustomWorld, size: number, userKey: string) {
    await attemptAsync(this, async () => {
      await saveProfileImage(this, userKey, size * 1024, 'image/png');
    });
  },
);

When(
  'I save a {int}KB JPEG profile image for the user',
  async function (this: CustomWorld, size: number) {
...
When(
  'I attempt to save a {int}KB JPEG profile image for the user',
  async function (this: CustomWorld, size: number) {
    await attemptAsync(this, async () => {
      await saveProfileImage(this, undefined, size * 1024, 'image/jpeg');
    });
...
When(
  'I save a {int}MB PNG profile image for the user',
  async function (this: CustomWorld, size: number) {
...
When(
  'I attempt to save a {int}MB PNG profile image for the user',
  async function (this: CustomWorld, size: number) {
    await attemptAsync(this, async () => {
      await saveProfileImage(this, undefined, size * 1024 * 1024, 'image/png');
    });
...
When(
  'I save a {int}MB JPEG profile image for the user',
  async function (this: CustomWorld, size: number) {
...
When(
  'I attempt to save a {int}MB JPEG profile image for the user',
  async function (this: CustomWorld, size: number) {
    await attemptAsync(this, async () => {
      await saveProfileImage(this, undefined, size * 1024 * 1024, 'image/jpeg');
    });
...
When('I attempt to save a GIF profile image for the user', async function (this: CustomWorld) {
  await attemptAsync(this, async () => {
    await saveProfileImage(this, undefined, 100 * 1024, 'image/gif');
  });
});
...
Then(
  'the profile image save should fail with {string}',
  async function (this: CustomWorld, expectedMessage: string) {
    const { error } = this.getLastError() as { error?: Error; errorMessage?: string };
    expect(error).toBeDefined();
    return await assert(() => {
      expect(error?.message.startsWith(expectedMessage)).toBe(true);
    }, `Expected error message to start with "${expectedMessage}", but got "${error?.message}"`);
  },
);

Given(
  'the user has a {int}KB PNG profile image saved',
  async function (this: CustomWorld, size: number) {
    await saveProfileImage(this, undefined, size * 1024, 'image/png');
  },
);
...
When(
  "I change the user's password to {string}",
...
When(
  'I attempt to change the password of user {string} to {string}',
...
When(
  'I attempt to get the profile for user {string}',
...
When(
  'I request users with the permissions {string}',
...
When(
  'I attempt to request users with the permissions {string}',
  async function (this: CustomWorld, permissions: string) {
    await attemptAsync(this, async () => {
      await getUsersWithPermissions(this, permissions);
    });
...
Given(
  'the role has been assigned to user {string}',
  async function (this: CustomWorld, userKey: string) {
...
Then(
  'I should receive a list containing the user {string}',
  function (this: CustomWorld, userKey: string) {
    const usersWithPermissions =
      (this.getStateReturn('getUsersWithPermissions') as
