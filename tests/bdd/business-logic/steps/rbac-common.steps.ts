...
Given(
  'a role {string} exists with the permissions {string}',
  async function (this: CustomWorld, roleKey: string, permissions: string) {
    await createRole(this, roleKey, parsePermissionList(permissions));
  },
);

Given(
  'a role exists with the permissions {string}',
  async function (this: CustomWorld, permissions: string) {
    await createRole(this, undefined, parsePermissionList(permissions));
  },
);

Given(
  'a role exists with display name {string} and hidden status {string}',
  async function (this: CustomWorld, displayName: string, hiddenStatus: string) {
...
Given(
  'a role {string} exists with display name {string} and hidden status {string}',
  async function (this: CustomWorld, roleKey: string, displayName: string, hiddenStatus: string) {
...
Given(
  'no users have the system role {string} assigned',
  async function (this: CustomWorld, roleStub: string) {
    const system = getSystemPersona(this);
    const role = await system.getRoleByStub(roleStub);
    if (!role) {
      throw new Error(`Expected system role with stub ${roleStub} to exist`);
    ...
    if (assignments.length > 0) {
      throw new Error(
        `Role ${roleStub} is already assigned to ${assignments.length} user(s), but the test expects no assignments`,
      );
    }
...
Given(
  'a user exists with the role {string} assigned',
  async function (this: CustomWorld, roleKey: string) {
    const system = getSystemPersona(this);
...
Given(
  'a user {string} exists with no roles assigned',
...
Given(
  'a user {string} exists with the role {string} assigned',
  async function (this: CustomWorld, userKey: string, roleKey: string) {
    const system = getSystemPersona(this);
    const role = (await getRoleByKey(this, roleKey)) as { id: number };
    const user = await createUser(this);
    await system.assignRoleToUser(user.id, role.id);
...
Given('no users have the role assigned', async function (this: CustomWorld) {
  ...
  if (assignments.length > 0) {
    throw new Error(
      `Role ${role.id} is already assigned to ${assignments.length} user(s), but the test expects no assignments`,
    );
  }
});

Given(
  'the role {string} has permissions {string}',
  async function (this: CustomWorld, roleKey: string, permissions: string) {
    const role = (await getRoleByKey(this, roleKey)) as { id: number };
    const system = getSystemPersona(this);
...
When(
  'I rename the role to have display name {string}',
...
When(
  'I attempt to rename the role to have display name {string}',
  async function (this: CustomWorld, displayName: string) {
...
When(
  'I rename the role to have display name {string} and hidden status {string}',
  async function (this: CustomWorld, displayName: string, hiddenStatus: string) {
...
When(
  'I attempt to rename the role to have display name {string} and hidden status {string}',
  async function (this: CustomWorld, displayName: string, hiddenStatus: string) {
...
When(
  'I duplicate the role {string} to create a new role {string}',
  async function (this: CustomWorld, sourceRoleKey: string, targetRoleKey: string) {
    await duplicateRole(this, sourceRoleKey, targetRoleKey);
  },
);

When(
  'I attempt to duplicate the role {string} to create a new role {string}',
  async function (this: CustomWorld, sourceRoleKey: string, targetRoleKey: string) {
...
When(
  'I attempt to delete the system role {string}',
  async function (this: CustomWorld, roleStub: string) {
    const system = getSystemPersona(this);
    const role = await system.getRoleByStub(roleStub);
    this.clearLastError();
...
When(
  'I create a role with the permissions {string}',
  async function (this: CustomWorld, permissions: string) {
    await createRole(this, undefined, parsePermissionList(permissions));
  },
);
...
When(
  'I attempt to create a role with the permissions {string}',
  async function (this: CustomWorld, permissions: string) {
    await attemptAsync(this, async () => {
      await createRole(this, undefined, parsePermissionList(permissions));
    });
...
When(
  "I update the role's permissions to be {string}",
  async function (this: CustomWorld, permissions: string) {
...
When(
  "I attempt to update the role's permissions to be {string}",
  async function (this: CustomWorld, permissions: string) {
...
Then(
  'the role should have the permissions {string}',
  async function (this: CustomWorld, permissions: string) {
    const role = getStoreRole(this);
    const system = getSystemPersona(this);
...
Then(
  'the user should have exactly the permissions originally assigned to the role',
...
Then(
  "the user's roles should be exactly {string}",
  async function (this: CustomWorld, expectedRoles: string) {
    const user = getStoreUser(this);
    const system = getSystemPersona(this);
    ...
    const expectedKeys = expectedRoles
      .split(',')
...
Then(
  'the permissions for the user should be exactly {string}',
  async function (this: CustomWorld, expectedPermissions: string) {
    const user = getStoreUser(this);
    const system = getSystemPersona(this);
    const actualPermissions = await system.getUserPermissions(user.id);
...
Then(
  'the role should have display name {string}',
  async function (this: CustomWorld, displayName: string) {
    const role = getStoreRole(this);
    const system = getSystemPersona(this);
    const roleDetails = await system.getRoleById(role.id);
...
Then(
  'the role {string} should have the permissions {string}',
  async function (this: CustomWorld, roleKey: string, permissions: string) {
    const role = await getRoleByKey(this, roleKey);
    const system = getSystemPersona(this);
    const actualPermissions = await system.getRolePermissions(role.id);
...
Then(
  'the role {string} should not be marked as a system role',
  async function (this: CustomWorld, roleKey: string) {
    const role = await getRoleByKey(this, roleKey);
    const system = getSystemPersona(this);
    const roleDetails = await system.getRoleById(role.id);
...
Then(
  'the role {string} should not be marked as hidden',
  async function (this: CustomWorld, roleKey: string) {
    const role = await getRoleByKey(this, roleKey);
    const system = getSystemPersona(this);
    const roleDetails = await system.getRoleById(role.id);
...
Then(
  'the role {string} should have display name {string}',
  async function (this: CustomWorld, roleKey: string, displayName: string) {
    const role = await getRoleByKey(this, roleKey);
    const system = getSystemPersona(this);
    const roleDetails = await system.getRoleById(role.id);
    expect(roleDetails).toBeTruthy();
...
Then(
  'the role {string} should be marked as hidden',
  async function (this: CustomWorld, roleKey: string) {
    const role = await getRoleByKey(this, roleKey);
    const system = getSystemPersona(this);
    const roleDetails = await system.getRoleById(role.id);
...
Then('an AuthenticationError should be thrown', async function (this: CustomWorld) {
  ...
  }, `Expected an AuthenticationError, but got ${error.name} with message: ${error.message}`);
});
...
Then('an AuthorizationError should be thrown', async function (this: CustomWorld) {
  ...
  }, `Expected an AuthorizationError, but got ${error.name} with message: ${error.message}`);
});
...
When(
  'I find all users with the role {string}',
  async function (this: CustomWorld, roleKey: string) {
    const system = getSystemPersona(this);
    const role = await getRoleByKey(this, roleKey);
...
Then(
  'the results should include the role {string}',
  async function (this: CustomWorld, roleKey: string) {
...
Then(
  'the results should include the user {string}',
  async function (this: CustomWorld, userKey: string) {
...
Then(
  'the results should not include the user {string}',
  async function (this: CustomWorld, userKey: string) {
    const expectedUser = getStoreUser(this, userKey);
    const userIds = this.getStateReturn('getUsers') as number[];
    expect(userIds).toBeTruthy();
...
Then(
  'the results should include exactly these permissions:',
...
When(
  'I attempt to get the system role {string}',
  async function (this: CustomWorld, roleStub: string) {
    await attemptAsync(this, async () => {
      await getSystemRole(this, roleStub);
    });
